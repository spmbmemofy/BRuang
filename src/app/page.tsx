...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [roomsRes, itemsRes, bookingsRes] = await Promise.all([
        fetch('/api/rooms'),
        fetch('/api/items'),
        fetch(`/api/bookings?date=${todayStr}`)
      ]);

      if (!roomsRes.ok || !itemsRes.ok || !bookingsRes.ok) {
  ...
  const handleAddItemSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitSuccess(true);
      setItemName(''); setItemCategory(''); setItemLocation(''); setItemDescription('');
      await fetchData();
            ...
            <form onSubmit={handleAddItemSubmit} className="modal-body-form">
              ...
              <div className="form-row">
                ...
                <div className="form-group flex-1">
                  ...
                  <input type="text" placeholder="Contoh: Ruang Server, Lantai 2" className="form-input" value={itemLocation} onChange={(e) => setItemLocation(e.target.value)} required />
                </div>
              </div>
              <div className="form-group">
